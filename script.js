...
function buildBottomNav(active){
  const items=[{href:'index.html',icon:'🏠',label:'หน้าแรก',key:'home'},{href:'category.html',icon:'🍽️',label:'หมวดหมู่',key:'cats'},{href:'add-recipe.html',icon:'➕',label:'เพิ่มเมนู',key:'add'},{href:'add-ingredient.html',icon:'🥕',label:'วัตถุดิบ',key:'ing'},{href:'about.html',icon:'ℹ️',label:'เกี่ยวกับ',key:'about'}];
...
// ===== Render category (with in-category search) =====
function renderCategory(){
    ...
    <div class="container">
      <div class="hero">
        <div class="panel">
          ...
          ${cat?`
          <div class="search-row" style="margin:10px auto">
            <div class="input"><span>🔎</span><input id="catSearch" type="text" placeholder="ค้นหาในหมวด${catInfo?catInfo.label:'นี้'}..."></div>
        ...
        <div class="section">
          <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:10px">
            ${cat?`<h2 style="margin:0">เมนูทั้งหมด</h2>`:'<h2 style="margin:0">เมนูทั้งหมด</h2>'}
  ...
  function renderCatList(nameQ){
    const list=$('#catList');if(!list)return;
    const by=$('#catSortSel')?.value||'new';
    let r=filterRecipes(recipes,[],'',(nameQ||''),cat);
    r=sortRecipes(r,by);
  ...
  $('#catSortSel')?.addEventListener('change',()=>renderCatList($('#catSearch')?.value||''));
  $('#catSearchBtn')?.addEventListener('click',()=>renderCatList($('#catSearch')?.value||''));
  $('#catSearch')?.addEventListener('input',()=>renderCatList($('#catSearch')?.value||''));
  $('#catSearch')?.addEventListener('keydown',(e)=>{if(e.key==='Enter'){e.preventDefault();renderCatList($('#catSearch')?.value||'');}});
}
...
function renderAbout(){
      ...
      <div class="panel">
        ...
        <div class="card" style="margin-top:12px">
          ...
          <ul class="list" style="margin-top:10px">
            <li>ค้นหาชื่อเมนูตรงๆ เช่น "กะเพรา" "ต้มยำ" — พิมพ์แล้วขึ้นทันที</li>
            <li>เลือกวัตถุดิบแล้วกดค้นหา พิมพ์ "กล้วย" ก็เจอ "กล้วยน้ำว้า" ได้</li>
            <li>ค้นหาในหมวดหมู่ได้ เช่น เข้าหมวดทอด แล้วพิมพ์ชื่อเมนูได้เลย</li>
            <li>กดหัวใจ ❤️ บนการ์ดเพื่อบันทึกเมนูโปรด</li>
            <li>กดดาวในหน้ารายละเอียดเพื่อให้คะแนน</li>
            <li>กดจับเวลาแต่ละขั้นตอนในหน้าวิธีทำ</li>
            <li>ปรับจำนวนคนเสิร์ฟ ปริมาณวัตถุดิบจะปรับตามอัตโนมัติ</li>
          </ul>
        </div>
        <div style="margin-top:12px">
          <button class="btn btn-primary" id="notifBtn">${notifGranted?'✅ เปิดการแจ้งเตือนแล้ว':'🔔 เปิดการแจ้งเตือน'}</button>
        </div>
      </div>
    </div></div>
  ...
  `;
  bindMobileMenu();
  $('#notifBtn')?.addEventListener('click',async()=>{await requestNotification();const b=$('#notifBtn');if(b&&Notification.permission==='granted'){b.textContent='✅ เปิดการแจ้งเตือนแล้ว';}});
}
